...
class Mat3 {
  ...
  #elements = new Float32Array(9).fill(0);
   ...
   */
  identity() {
    this.fromArray([1, 0, 0, 0, 1, 0, 0, 0, 1]);
    return this;
  }
  ...
  multiply(other) {
    if (!(other instanceof Mat3)) {
      throw new TypeError('Parameter "other" must be an instance of Mat3.');
    }
  ...
  transpose() {
    ...
    this.fromArray([e[0], e[3], e[6], e[1], e[4], e[7], e[2], e[5], e[8]]);
    return this;
  }
  ...
  scale(vector) {
    if (!(vector instanceof Vec2)) {
      throw new TypeError('Parameter "vector" must be an instance of Vec2.');
    }
    const sx = vector.x;
    const sy = vector.y;
  ...
  rotate(angle) {
    ...
    const a01 = this.#elements[3];
    const a02 = this.#elements[6];
    const a10 = this.#elements[1];
    const a11 = this.#elements[4];
    const a12 = this.#elements[7];

    this.#elements[0] = a00 * c + a10 * s;
    this.#elements[1] = -a00 * s + a10 * c;
    this.#elements[3] = a01 * c + a11 * s;
    this.#elements[4] = -a01 * s + a11 * c;
    this.#elements[6] = a02 * c + a12 * s;
    this.#elements[7] = -a02 * s + a12 * c;
  ...
  fromArray(array) {
    if (!(array instanceof Float32Array) && !Array.isArray(array)) {
      throw new TypeError('Parameter "array" must be a Float32Array or an array of numbers.');
    }
    if (array.length !== 9) {
  ...
  static multiplyScalar(matrix, scalar) {
    if (!(matrix instanceof Mat3)) {
      throw new TypeError('Parameter "matrix" must be an instance of Mat3.');
    }
    if (typeof scalar !== "number") {
      throw new TypeError('Parameter "scalar" must be a number.');
    }
  ...
  static translate(matrix, vector) {
    ...
    return new Mat3([m[0], m[3], m[6] + tx, m[1], m[4], m[7] + ty, m[2], m[5], m[8]]);
  }
   ...
   * @param {Float32Array | number[]} array - Array of 9 numerical elements.
