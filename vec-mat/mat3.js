...
class Mat3 {
   ...
   * @param {Mat2|Mat3} other - The matrix to multiply with.
   * @returns {Mat3} This instance for chaining.
   */
  multiply(other) {
    validateMat2OrMat3(other, "other matrix");

    const otherElements = new Float32Array(9);

    if (other instanceof Mat3) {
      otherElements.set(other.#elements);
    } else {
      const mat2 = other.elements;

      otherElements.set([...mat2.slice(0, 2), 0, ...mat2.slice(2, 4), 0, ...[0, 0], 1]);
    }
   ...
   * @param  {...(Mat2|Mat3)} matrices - Matrices to multiply.
  ...
  static multiply(...matrices) {
    ...
    /* this is a bit uggly but the typechecker complains that result is of type Mat2 and Mat3,
    and return type of .reduce is also of type Mat2 or Mat3 */
    return /**@type {Mat3}*/ (
      matrices.reduce(
        /**@param {Mat3} r*/ (r, m) => {
          return r.multiply(m);
