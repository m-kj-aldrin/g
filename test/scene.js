...
function square(p, s) {
    ...
    function render(ctx, projector) {
        let projectedVerts = verts.map((v) =>
            projector(v.multiply(modelMatrix).scalarMultiply(s))
        );
...
sq0.render(canvas.context, (v) =>
    project(viewMatrix, projectionMatrix, viewPortMatrix, v)
);
